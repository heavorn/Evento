...
class SignupController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    let signinButton: UIButton = {
        ...
        attributeTitle.append(NSAttributedString(string: "Sign In", attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 14), NSAttributedString.Key.foregroundColor: UIColor.mainColor()]))
            ...
            storage.putData(uploadData, metadata: nil, completion: { (metadata, error) in
                ...
                storage.downloadURL(completion: { (url, error) in
                    ...
                    if let profileUrl = url?.absoluteString {
                        let values = ["name": name, "email": email, "profileUrl": profileUrl, "phone": ""]
    ...
    @objc func handleTextInputChange(){
        ...
        if isFormValid {
            ...
            signUpButton.backgroundColor = UIColor.rgb(red: 224, green: 83, blue: 83, alpha: 0.5)
